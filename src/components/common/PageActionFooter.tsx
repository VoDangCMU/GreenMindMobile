...
interface PageActionFooterProps {
    /**
     * Main central action (Scan)
     */
    onScan: () => void;
    scanIcon?: React.ReactNode;

    /**
     * Right side action (Import)
     */
    onImport?: (() => void) | null;
    ...
    /**
     * Left side action (History)
     */
    onHistory?: () => void;
    historyLabel?: string;
}

const PageActionFooter: React.FC<PageActionFooterProps> = ({
    onScan,
    ...
    return (
        ...
        >
            ...
            <button
                onClick={onHistory}
                disabled={!onHistory}
                className={`flex flex-col items-center ${onHistory ? "text-greenery-700 hover:text-greenery-900" : "text-greenery-300"
                    }`}
            >
                <svg
                ...
                <span className="text-xs font-medium">{historyLabel}</span>
            </button>

            {/* Center Action (Scan) - Absolute positioned */}
            ...
            {onImport ? (
                ...
                /* Spacer to balance layout if no right action */
