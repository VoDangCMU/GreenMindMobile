...
import { getQuestionSetById, type ISurveyResponse } from "@/apis/backend/v2/survey";
...
export default function QuizPage() {
  ...
  const getQuestionSet = async () => {
    ...
    call({
      fn: () => getQuestionSetById(qsId!),
      onSuccess: async (data: ISurveyResponse) => {
        // Set rawQuestion with the v2 response (cast to compatible type)
        setRawQuestion(data as any);

        const flatQuestions: Question[] = (data.data.questions || []).map((q) => ({
          id: q.id,
          question: q.question,
          behaviorNormalized: q.behaviorNormalized,
          options: (q.questionOptions || []).map((opt) => ({
